...
results = model.predict(source="resources/videos/23_NT_R1_J7_P9_P10.mp4",save=True)
...
for r in results:
    ...
    i = 0
    boxes = r.obb.xywhr.tolist() #Formato de salida tipo [centroidex,centroidey,areax,areay]
    confidences = r.obb.conf.tolist()
    for bb in boxes:
        #X Boxes por
...
"""
#RightFinal = scipy.ndimage.laplace(RightFinal)
#DareaList = scipy.ndimage.laplace(DareaList)
...
fig, ax1 = plt.subplots()
ax2 = ax1.twinx()
#LeftFinal = scipy.ndimage.laplace(LeftFinal)
#IareaList = scipy.ndimage.laplace(IareaList)
